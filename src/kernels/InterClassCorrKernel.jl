...
matrix will be n x m  where 
  ...
  code adapted from 
    ...
    """
function assignWorkToCooperativeBlocks(slicesNumb )
#     kernel = @cuda launch=false kernelFun(args...) 
#    CUDA.active_blocks(kernel.fun, 256)  - it evaluates to 1 - hence it should be irrespective to machine
   numberOfBlocks = attribute(device(), CUDA.DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT)
